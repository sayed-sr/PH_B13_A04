...
function renderUI() {
    ...
    // Updating Statistics numbers in the stat-ctn
    document.getElementById('total-count').innerText = jobs.length;
    document.getElementById('interview-count').innerText = jobs.filter(j => j.status === 'interview').length;
    document.getElementById('rejected-count').innerText = jobs.filter(j => j.status === 'rejected').length;
    document.getElementById('display-count').innerText = filtered.length;

    // Check if the current filter result is empty
    if (filtered.length === 0) {
        wrapper.innerHTML = "";
        emptySec.classList.remove('hidden'); // Shows your PNG section
    } else {
        emptySec.classList.add('hidden'); // Hides your PNG section
        wrapper.innerHTML = filtered.map(job => {
            let tagTxt = "NOT APPLIED";
            let tagClass = "";
            
            // Dynamic labeling based on button clicks
...
// Moves card to correct tab and changes tag text
function updateStatus(id, stat) {
    jobs = jobs.map(j => j.id === id ? {...j, status: stat} : j);
    renderUI();
}

// Removes card completely from the array
function deleteJob(id) {
    jobs = jobs.filter(j => j.id !== id);
    renderUI();
}

// Event Listeners for Tab buttons (All, Interview, Rejected)
...
// Initial execution to show the 8 cards on load
renderUI();